...
if __name__ == "__main__":
    ...
    trc_output_folder = './output/label_propagation/TRCs/'
    community_output_folder = './output/label_propagation/Communities/'
    ...
    clear_output_folder(trc_output_folder)  # clear existing files  
    clear_output_folder(community_output_folder) # clear existing files  

    word_to_community, community_words = analyze_results()

    trc_community_counter = defaultdict(int)  # for visu
    community_to_trcs = defaultdict(list)  # mapping
    ...
    for bar in bars:
        yval = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2, yval, f"{round(yval/total_trcs*100, 1)}%", ha='center', va='bottom') # percentage annotation
    ax2 = plt.gca().twinx()
